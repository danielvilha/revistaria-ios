...
//Rare exceptions to indicate a bug in the _revistariaios code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_revistariaios = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_revistariaios]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.revistariaios.module"
